terragrunt = {
  remote_state {
    backend = "gcs"
    config {
      bucket  = "${get_env("TF_VAR_gcp_sandbox_states_bucket", "`TF_VAR_gcp_sandbox_states_bucket` not set")}"
  ...
  terraform {
    ...
    extra_arguments "retry_lock" {
      ...
      arguments = [
        ...
        "--var-file=${get_tfvars_dir()}/../envars/${get_env("TF_VAR_environment", "`TF_VAR_environment` not set")}.tfvars",
